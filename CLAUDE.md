...
## 7. Amazon Affiliate Components (Custom HTML)
MicroCMSのHTMLブロック等でAmazon物販（ルーター、LANケーブル等）を紹介する際、専用のCSSクラス（`src/app/globals.css` の `.affi-frame` 等）が用意されています。
1〜3カラムのレイアウトに対応しており、サイバーパンクデザインにマッチする高品質なアフィリエイト枠を出力できます。

**使用例（2カラム〜3カラムの場合）**:
```html
<div class="affi-frame">
  <div class="affi-title-row">
    <div class="affi-title">おすすめゲーミング<span class="accent">ルーター</span></div>
    <div class="affi-subtitle">// RECOMMENDED GEAR</div>
  </div>
  <div class="affi-grid"> <!-- 1カラムの場合は class="affi-grid cols-1" とする -->
    <div class="affi-card featured">
      <div class="affi-card-head">
        <span class="affi-rank">1ST PICK</span>
        <span class="affi-best">BEST BUY</span>
      </div>
      <div class="affi-thumb"><img src="画像URL" alt=""></div>
      <div class="affi-card-body">
        <div class="affi-name">商品名</div>
        <div class="affi-spec">
          <span class="affi-spec-tag">Wi-Fi 6</span>
        </div>
        <div class="affi-price-row">
          <span class="affi-price-label">PRICE:</span>
          <span class="affi-price">¥24,800</span>
        </div>
        <a href="Amazonのリンク" class="affi-btn" target="_blank" rel="sponsored">
          <span class="a-logo">Amazon</span> で見る
        </a>
      </div>
    </div>
  </div>
...
```
Claudeが今後物販ページ（`/gear`など）を新規作成する際も、この `affi-` 系クラス群を使い回すことで、全体のデザインを統一できます。
